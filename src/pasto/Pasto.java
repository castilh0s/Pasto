...
/**
 * Um pasto contém ovelhas, lobos, cercas, plantas e possivelmente outras
 * entidades. Estas entidades movem-se pelo pasto e tentam achar comida, outras
 * entidades do mesmo tipo e fogem de possíveis inimigos.
 */
public class Pasto {
	...
	private int lobos;
	private int ovelhas = 10;
	private int plantas = 25;
	private int cercas;

	private Set<Entidade> mundo = new HashSet<Entidade>();
	private Map<Point, List<Entidade>> grid = new HashMap<Point, List<Entidade>>();
	private Map<Entidade, Point> ponto = new HashMap<Entidade, Point>();

	private PastoGUI gui;

	/**
	 * Cria uma nova instância desta classe e posiciona as entidades nela, em
	 * posições aleatórias.
	 */
	public Pasto() {
		...
		/*
		 * O pasto é rodeado por uma cerca. Substitua Dummy por Cerca quando você criar
		 * essa classe
		 */
		...
		/*
		 * Agora insira o número correto de entidades no pasto. Note que quando você
		 * criar suas entidades, você terá que fazer um for para cada animal, utilizando
		 * a variável em questao: lobos, ovelhas, plantas, etc.
		 */
...
//		}
	 ...
	 * 
	 * Se a primeira posição aleatória estiver ocupada, o resto é feita uma busca no
	 * resto da tela para encontrar uma posição livre.
	 */
	private Point getPosicaoLivre(Entidade aPosicionar) throws MissingResourceException {
		Point posicao = new Point((int) (Math.random() * largura), (int) (Math.random() * altura));
		...
		throw new MissingResourceException("Não há espaço livre" + " no pasto", "Pasto", "");
	}
